...
public class Storage {
    ...
    public static func save<T: Encodable>(_ object: T, filename: String) {
        store(object, to: .documents, as: filename)
    }

    public static func cache<T: Encodable>(_ object: T, filename: String) {
        store(object, to: .caches, as: filename)
    }

    public static func saved<T: Decodable>(_ filename: String, as type: T.Type) -> T? {
        return retrieve(filename, from: .documents, as: type)
    }

    public static func cached<T: Decodable>(_ filename: String, as type: T.Type) -> T? {
    ...
    ///   - type: struct type (i.e. Message.self)
    /// - Returns: decoded struct model(s) of data
    static func bundled<T: Decodable>(_ fileName: String, as type: T.Type) -> T? {
        guard let path = Bundle.main.path(forResource: "TestStates", ofType: "json") else {
            return nil
        }
